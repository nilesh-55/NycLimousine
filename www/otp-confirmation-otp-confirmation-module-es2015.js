...
let OtpConfirmationPage = class OtpConfirmationPage {
    constructor(router, route, envservice, loadingController) {
        this.router = router;
        this.route = route;
    ...
    enterOtp() {
        this.presentLoading().then(a => {
            this.envservice.confirmOtp(this.email, this.otp).subscribe((data) => {
    ...
    getnewPassword() {
        this.presentLoading().then(a => {
            ...
            if (this.newEnterPassword == this.re_enterPassword) {
                this.envservice.getNewPassword(this.re_enterPassword, this.email).subscribe((data) => {
...
OtpConfirmationPage.ctorParameters = () => [
...
//# sourceMappingURL=otp-confirmation-otp-confirmation-module-es2015.js.map